...
open class SPKHybridContext: NSObject, NSCopying {
    ...
    open func merge(withContext context: SPKHybridContext?, isOverride: Bool = false) {
        ...
        if self.globalProps is [String: Any] {
            self.globalProps = Self.merge(
                withDict: context.globalProps as? [String: Any],
                to: self.globalProps as? [String: Any], isOverride: isOverride)
        } else {
            self.globalProps = Self.merge(
                withProp: context.globalProps,
                to: self.globalProps,
        ...
        self.schemeParams =
            Self.merge(
                withProp: context.schemeParams,
                to: self.schemeParams,
                isOverride: isOverride) as? SPKHybridSchemeParam

        self.originURL =
            Self.merge(
                withProp: context.originURL,
                to: self.originURL,
                isOverride: isOverride) as? String

        self.fullURL =
            Self.merge(
                withProp: context.fullURL,
                to: self.fullURL,
                isOverride: isOverride) as? String

        self.extra =
            Self.merge(
                withProp: context.extra,
                to: self.extra,
        ...
        self.widthMode =
            Self.merge(
                withProp: context.widthMode,
                to: self.widthMode,
                isOverride: isOverride) as? NSNumber

        self.heightMode =
            Self.merge(
                withProp: context.heightMode,
                to: self.heightMode,
                isOverride: isOverride) as? NSNumber

        self.initialData =
            Self.merge(
                withProp: context.initialData,
                to: self.initialData,
